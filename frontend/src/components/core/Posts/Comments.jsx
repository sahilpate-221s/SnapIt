        ...
        {comments.length > 0 ? (
          [...comments]
            ...
            .map((comment, index) => (
              <div
                key={index}
                className="flex items-start space-x-4 bg-gray-50 p-3 rounded-lg hover:shadow-lg transition-shadow duration-300"
              >
                <img
                  src={comment.user?.profilePicture || "default-profile-pic-url"}
                  alt={comment.user?.name || "User"}
                  className="w-12 h-12 rounded-full object-cover"
                />
                <div className="flex flex-col">
