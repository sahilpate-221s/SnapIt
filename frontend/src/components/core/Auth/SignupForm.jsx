...
const SignupForm = () => {
  ...
  const handleOnSubmit = async (e) => {
    ...
    setSubmitting(true);
    await dispatch(register(name, email, password, confirmPassword, navigate));
    setSubmitting(false);
  };

  return (
    <div className="flex justify-center items-center mt-1">
      <div className="bg-white rounded-lg p-3 w-full max-w-md sm:w-[90%] sm:mx-6">
        <form onSubmit={handleOnSubmit} className="flex flex-col space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={handleOnChange}
              placeholder="Your full name"
              className="w-full p-2 pr-10 border border-gray-400 rounded-lg
                         focus:outline-none focus:ring-2 focus:ring-gray-800
                         focus:border-transparent"
            />
            <span className="absolute right-3 top-11 -translate-y-1/2 text-gray-400">
              👤
            </span>
          </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-2 px-6 text-white font-semibold rounded-lg
