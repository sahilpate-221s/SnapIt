...
export const fetchAllPosts = (page = 1, limit = 10) => {
  return async (dispatch, getState) => {
    console.log(`API Call: fetchAllPosts - Page ${page}, Limit ${limit}`);
    console.time(`fetchAllPosts-page-${page}`);

    try {
      const response = await axiosInstance.get(
        `${ALL_POSTS_API}?page=${page}&limit=${limit}`,
        {
          headers: {
      ...
      console.timeEnd(`fetchAllPosts-page-${page}`);

      if (!response.data.success) {
        throw new Error(response.data.message);
      }

      const existing = getState().posts.posts || [];
      console.log(`Existing posts count: ${existing.length}, New posts count: ${response.data.posts.length}`);

      if (page === 1) {
        // Replace for initial load
        dispatch(setPosts(response.data.posts));
        console.log('Replaced posts for initial load');
      } else {
        // Append for subsequent loads
        dispatch(setPosts([...existing, ...response.data.posts]));
        console.log('Appended posts for infinite scroll');
      }
